...
@pytest.fixture()
def wellsfargo_checking_test_setup(request):
    request.cls.payment_method = models.PaymentMethod.objects.get(
...
def _get_all_sorted_receipts():
    q_receipts = (
        models.Transaction.objects.extra(
            select={
...
class BaseTestItemize:
    ...
    def _run_itemizer(self, transactions: RawTransactionIterable):
        self.itemizer.process_transactions(transactions)
        assert not log_contains_message(
            self.mock_logger, "Pattern not found", level=logging.WARNING
        )
...
class TestBmoBankAccountItemize(BaseTestItemize):
    def _make_transaction(self):
        return functools.partial(
            _make_expected_transaction,  # pylint:disable=invalid-name
    ...
    def test_hst_adjustments(self):
        ...
        adjustments = models.TaxAdjustment.objects.order_by(
...
class TestBmoCreditItemize(BaseTestItemize):
    ...
    def test_payment_exclusions(self):
        ...
        transactions = [
            ...
            _T(
                4, "2016-09-09", -45377, "INTEREST ADVANCES  @ 02.70000% TO 09SEP", misc
            ),
        ]
