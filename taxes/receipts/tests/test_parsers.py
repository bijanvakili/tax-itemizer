...
from taxes.receipts.types import Currency, RawTransactionSequence
from taxes.receipts.models import PaymentMethod
from taxes.receipts import parsers
...
def _make_expected_transaction(
    line_number: int,
    transaction_datestr: str,
    amount: int,
    description: str,
    misc: dict,
    currency: Currency = None,
    payment_method: PaymentMethod = None,
) -> parsers.RawTransaction:
...
class TestParsers:
    ...
    def _run_parser(
        self, filename
    ) -> RawTransactionSequence:  # pylint:disable=redefined-outer-name
        test_parser = self.parser_factory.get_parser(filename)
        results = list(
            test_parser.parse(os.path.join(self.transaction_fixture_dir, filename))
        )

        assert not log_contains_message(
            self.mock_logger, "Pattern not found", level=logging.ERROR
        )
    ...
    def test_bmo_savings_parser(self):
        ...
        _T = functools.partial(
            _make_expected_transaction,
        ...
        def _m(transaction_code: str) -> dict:
            return {"last_4_digits": "0066", "transaction_code": transaction_code}
    ...
    def test_bmo_readiline_parser(self):
        ...
        assert self._run_parser("bmo_readiline_2016-09.csv") == [
            ...
            _T(
                ...
                "2016-09-09",
                -45377,
                "INTEREST ADVANCES  @ 02.70000% TO 09SEP",
                expected_misc,
            ),
        ]

    def test_bmo_mastercard_parser(self):
        expected_payment_method = PaymentMethod.objects.get(
        ...
        assert self._run_parser("bmo_mastercard.csv") == [
            ...
            _T(
                ...
                "2016-05-11",
                -2066,
                "YYZ BOCCONE PRONTO MISSISSAUGA ON",
                expected_misc,
            ),
            ...
            _T(
                ...
                "2016-04-22",
                -85276,
                "AIR CAN 22142161633944 WINNIPEG MB",
                expected_misc,
            ),
            ...
            _T(
                ...
                "2016-05-07",
                -13300,
                "PIZZERIA LIBRETTO - DA TORONTO ON",
                expected_misc,
            ),
            _T(
                11,
                "2016-05-07",
                -4028,
                "JOE'S HAMBURGERS RICHMOND HILLON",
                expected_misc,
            ),
            _T(12, "2016-05-08", -6734, "REAL MO MOS TORONTO ON", expected_misc),
            _T(
                13, "2016-05-08", -369, "TIM HORTONS 3021 QTH TORONTO ON", expected_misc
            ),
            ...
            _T(
                ...
                "2016-05-10",
                -2108,
                "PANERA BREAD CAFE# 620 NORTH YORK ON",
                expected_misc,
            ),
    ...
    def test_chase_visa_parser(self):
        ...
        assert self._run_parser("chase_visa_2019-02.csv") == [
            ...
            _T(
                6, "2019-02-24", -4283, "HODALA RESTAURANT.", _m("Food & Drink", "Sale")
            ),
            _T(
                7,
                "2019-02-03",
    ...
    def test_wellsfargo_checking_parser(self):
        ...
        assert self._run_parser("wellsfargo_checking_2016-08.csv") == [
            _T(
                ...
                "2016-08-31",
                16636,
                "GUSTO PAY 666666 555555 6AAAAAA06uq John Doe",
                {},
            ),
            _T(
                2,
                "2016-08-30",
                -7666,
                "AUTOINSURANCE PNOT.DED. 555555 333264420000 DOE, JOHN",
                {},
            ),
            _T(
                3,
            ...
            _T(
                ...
                "2016-08-29",
                -23127,
                "ONLINE TRANSFER REF #IBE5SMGBDB TO SECURED CARD XXXXXXXXXXXX0002 ON "
                "08/27/16",
                {},
            ),
            ...
            _T(8, "2016-08-19", -32000, "CHECK # 136", {}),
            _T(
                9,
                "2016-08-18",
                -5480,
            ...
            _T(
                ...
                "2016-08-15",
                8035,
                "GUSTO PAY 555555 666666 6AAAAAAlbri John Doe",
                {},
            ),
