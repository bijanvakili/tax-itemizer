...
from taxes.settings.base import *  # noqa: F401,F403  # pylint: disable=wildcard-import
...
RECEIPTS_CONFIG_DIR = os.environ.get(
    "RECEIPTS_CONFIG_DIR", DEFAULT_CONFIG_DIR
)  # noqa: F405
RECEIPTS_CONFIG_PATH = os.path.join(  # noqa: F405
    RECEIPTS_CONFIG_DIR, f"config.{RECEIPTS_ENV}.yaml"
)

TEST_DATA_FIXTURE_DIR = os.path.join(
    os.getcwd(), "data", "fixtures", "tests"
)  # noqa: F405
...
if RECEIPTS_ENV == "test":
    ...
    DATABASES["default"] = {
        ...
        "TEST": {"ENGINE": "django.db.backends.postgresql_psycopg2"},
    }
else:
